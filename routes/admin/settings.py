...
def add_popup():
    if request.method == 'POST':
        try:
            ...
            logger.error(f"ValueError in add_popup: {e}")
        except Exception as e:
            db.session.rollback()
            flash('Erreur lors de l\'enregistrement. Veuillez vérifier les données.', 'error')
            logger.error(f"Error in add_popup: {e}")
...
def edit_popup(id):
    ...
    if request.method == 'POST':
        try:
            ...
            popup.ordering = int(request.form.get('ordering', 0) or 0)
            ...
            db.session.rollback()
            flash(f'Erreur de format: {str(e)}', 'error')
            logger.error(f"ValueError in edit_popup: {e}")
        except Exception as e:
            db.session.rollback()
            flash('Erreur lors de la mise à jour. Veuillez vérifier les données.', 'error')
            logger.error(f"Error in edit_popup: {e}")
...
def toggle_popup(id):
    try:
        ...
        db.session.rollback()
        logger.error(f"Error in toggle_popup: {e}")
...
def delete_popup(id):
    try:
        ...
        logger.error(f"Error in delete_popup: {e}")
    return redirect(url_for('admin.list_popups'))
